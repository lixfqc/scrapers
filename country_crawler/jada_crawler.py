...
class JadaCrawler(BaseCrawler):
    ...
    def latest_available_month(self):
        """探测JADA官网最新可用月份（品牌速报页面 /pages/364/）

        从页面第一个 /files/libs/ 的xlsx链接文本（如'ブランド別登録台数統計（2026年1月～7月）'）
        正则提取最新年月，返回 (year, month) 或 None。
        """
        try:
            resp = requests.get('https://www.jada.or.jp/pages/364/', headers=self.headers, timeout=30)
            if resp.status_code != 200:
                self.logger.error(f'JADA品牌速报页访问失败: HTTP {resp.status_code}')
                return None
            resp.encoding = resp.apparent_encoding if resp.encoding in ('ISO-8859-1', 'ascii') else resp.encoding
            for m in re.finditer(r'[（(](\d{4})年\d{1,2}月～(\d{1,2})月[)）]', resp.text):
                year = int(m.group(1))
                month = int(m.group(2))
                self.logger.info(f'JADA最新可用月份: {year}-{month:02d}')
                return (year, month)
            self.logger.warning('JADA页面未找到月份范围文本')
            return None
        except Exception as e:
            self.logger.error(f'JADA探测失败: {e}')
            return None

    def _current_url(self, page_id, current_fallback):
        """从JADA页面解析当年文件URL

        Args:
            page_id: 页面ID（'364'=品牌速报，'342'=燃料别）
            current_fallback: 硬编码兜底URL

        Returns:
            str: 当年文件URL
        """
        try:
            resp = requests.get(f'https://www.jada.or.jp/pages/{page_id}/', headers=self.headers, timeout=30)
            if resp.status_code == 200:
                m = re.search(r'href="([^"]*files/libs/[^"]+\.xlsx[^"]*)"[^>]*>[^<]*</a>', resp.text)
                if m:
                    url = m.group(1)
                    if url.startswith('http'):
                        return url
                    return 'https://www.jada.or.jp' + url
        except Exception as e:
            self.logger.warning(f'当年URL探测失败，使用兜底: {e}')
    ...
    def download_brand_excel(self, year):
        ...
        if year == datetime.now().year:
            url = self._current_url('364', JADA_BRAND_CURRENT_URL)
    ...
    def download_fuel_excel(self, year):
        ...
        if year == datetime.now().year:
            url = self._current_url('342', JADA_FUEL_CURRENT_URL)
