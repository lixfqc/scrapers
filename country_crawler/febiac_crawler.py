...
def _get_febiac_pdf_urls(data_year, data_month):
    ...
    URL格式规律（经实测验证，按时间顺序）：
    - 2024年: {MMYY} - Cars by make.pdf (MMYY格式，空格+连字符+空格)
    - 2025年上半年: 混合格式
        - 1月: {YYMM}_Cars by make.pdf (YYMM + 下划线 + 空格)
        - 6月: {MMYY} - Cars by make.pdf (MMYY格式)
    - 2025年11-12月: 新格式！月份缩写 + 年份
        - {month_abbr}{yy}-Car_Mpc_By_Brand_Pers.pdf
        - {month_abbr}{yy}-Car_Mpc_By_Brand_Pers_0.pdf
    - 2026年至今: 全下划线，YYMM格式
        - {YYMM}_Cars_by_make.pdf
    ...
    month_abbr = MONTH_ABBR.get(data_month, '')  # 月份缩写: 7月 -> jul
    yy = str(data_year)[2:]  # 年份后两位: 2026 -> 26
    ...
    # 格式1: 2025年11-12月新格式 - 月份缩写 + 年份 + Car_Mpc_By_Brand_Pers
    urls.append(f'{base_url}/{upload_dir}/{month_abbr}{yy}-Car_Mpc_By_Brand_Pers.pdf')
    urls.append(f'{base_url}/{upload_dir}/{month_abbr}{yy}-Car_Mpc_By_Brand_Pers_0.pdf')
    
    # 格式2: 2026年格式 - YYMM + 全下划线
    ...
    urls.append(f'{base_url}/{current_dir}/{month_abbr}{yy}-Car_Mpc_By_Brand_Pers.pdf')
