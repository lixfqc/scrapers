...
from base_crawler import DB_CONFIG
...
# ONS数据集发布页（从中动态探测最新文件URL）
ONS_DATASET_PAGE = 'https://www.ons.gov.uk/economy/economicoutputandproductivity/output/datasets/uknewvehicleregistrationsandproduction'
...
def discover_latest_url():
    """从ONS数据集发布页探测最新Excel文件URL"""
    try:
        resp = requests.get(ONS_DATASET_PAGE, headers=HEADERS, timeout=30)
        if resp.status_code == 200:
            resp.encoding = resp.apparent_encoding
            m = re.search(r'href="(/file\?uri=[^"]+\.xlsx[^"]*)"', resp.text)
            if m:
                return 'https://www.ons.gov.uk' + m.group(1)
    except Exception as e:
        logging.warning(f'ONS最新URL探测失败: {e}')
    return None
...
def download_ons_excel():
    ...
    # 先尝试从发布页探测最新URL
    latest = discover_latest_url()
    urls = [latest, ONS_URL_2026, ONS_URL_2025] if latest else [ONS_URL_2026, ONS_URL_2025]
...
def latest_available_month():
    """探测ONS数据最新可用月份（下载解析Excel取最大年月）"""
    try:
        content = download_ons_excel()
        records = parse_energy_data(content, start_year=2000)
        max_pair = None
        for r in records:
            if max_pair is None or (r['year'], r['month']) > max_pair:
                max_pair = (r['year'], r['month'])
        if max_pair:
            logging.info(f'ONS最新可用月份: {max_pair[0]}-{max_pair[1]:02d}')
            return (max_pair[0], max_pair[1])
    except Exception as e:
        logging.warning(f'ONS最新月份探测失败: {e}')
    return None
