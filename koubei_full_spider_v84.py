...
class FullKoubeiSpider:
    ...
    def init_database(self):
        try:
            ...
            print("数据库连接成功")
            try:
                backup_cs = f'postgresql+psycopg2://{self.db_config["user"]}:{self.db_config["password"]}@{self.db_config["host"]}:{self.db_config["port"]}/{self.db_config["dbname"]}'
                self.local_engine = create_engine(backup_cs)
                print("云端PG备份连接成功")
            except Exception as backup_e:
                print(f"云端PG备份连接失败: {backup_e}")
            return True
        except Exception as e:
    ...
    def append_to_data_koubei(self):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                # [V8.7新增] 同步序列，防止因序列落后导致主键冲突
                try:
                    max_id_result = conn.execute(text("SELECT MAX(id) FROM data_koubei"))
                    max_id = max_id_result.fetchone()[0]
                    if max_id:
                        conn.execute(text(f"SELECT setval('koubei_id_seq', {max_id + 1})"))
                        print(f"[序列同步] koubei_id_seq 已同步到 {max_id + 1}")
                except Exception as seq_e:
                    print(f"[序列同步] 跳过: {seq_e}")

                # [V8.7新增] 使用 ON CONFLICT DO NOTHING 防止主键冲突丢失数据
                conn.execute(text("""
